...
test('APP US - Footer - Our Products - Sexual Health', async t => {
  await t
    .expect(page.OPSH.innerText).eql('Sexual Health')
    .expect(page.OPSH.exists).ok()
});
test('APP US - Footer - Our Products - General Health', async t => {
  await t
    .expect(page.OPGH.innerText).eql('General Health')
    .expect(page.OPGH.exists).ok()
});
test('APP US - Footer - Our Products - Covid-19', async t => {
  await t
    .expect(page.OPCV.innerText).eql('Covid-19')
    .expect(page.OPCV.exists).ok()
});
