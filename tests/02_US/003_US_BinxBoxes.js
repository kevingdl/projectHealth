...
fixture ('BINX BOXES '+ MyEnv)
    .page(MyEnv+'binx-boxes')
...
test('APP US - Binx Box I - CG Price', async t => {
  await t.expect(page.BinxBoxI_Price.exists).ok()
});
...
test('APP US - Binx for PrEP - GAO CG, HIV, Shyphilis and Creatinine Text', async t => {
  await t.expect(page.BinxForPrEP_Text.innerText).eql('binx for PrEP\nTests for:\nGenital, Anal, and Oral Chlamydia and Gonorrhea, HIV, Syphilis, and Creatinine\nWho is this for?\nPeople on PrEP typically need quarterly STI screening as part of that regimen. This test satisfies quarterly STI screening requirements for HIV PrEP, it does not include the prescription component.')
});
